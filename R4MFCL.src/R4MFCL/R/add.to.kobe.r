add.to.kobe <- function(vars=grid.criteria[,2],grid.obj=grid.out,SBopt="latest")
{
    ...
    SB <- ifelse(SBopt=="latest","SBlatest.SBmsy","SBcurr.SBmsy")
    ...
    points(x,y, col=my.cols[i], pch=16, cex=1)
    }
}
