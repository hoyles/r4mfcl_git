...
# Cleaning
  ...
  # 2009 catch -> 2010 where none reported
  a <- c(JPf,KRf)
  m <- m[!(m[,4] %in% a & m[,1]==2010),]                               # remove JP, KR data for 2010(?). What about TW in R1?
  m <- rbind(m,cbind(2010,m[m[,4] %in% a & m[,1]==2009,c(2:106)]))     # replace with 2009 data
  m[m[,4] %in% a & m[,1]==2010,c(6,7)] <- -1

  # 2007 effort -> 2008 where none reported
  a <- c(NCf,FJf,OTf,SAf,TOf,20,24,26,29,30)
  m <- m[!(m[,4] %in% a & m[,1]==2010),]                               # remove
  m <- rbind(m,cbind(2010,m[m[,4] %in% a & m[,1]==2009,c(2:106)]))     # replace
  m[m[,4] %in% a & m[,1]==2010 & m[,2]>6,c(5,7)] <- -1
