...
fisheries=c(1,2,4,7,10,12),parfile=basepar,fleetlabs=BET_fleets$fnames,XLIM=c(1950,2011), N.rows=5, legpos="topright")
{
###########################################################################
...
#reg <- 1
par(mfrow=c(N.rows,2), mar=c(2,4,1,1))
...
legend(legpos, legend=c("Observed", "Predicted"), pch=c(19,NA), lty=c(NA,1), lwd=2, bty="n", col=c(grey(0.5), "black"))
}