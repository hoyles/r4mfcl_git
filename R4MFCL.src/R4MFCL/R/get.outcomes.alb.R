 get.outcomes.alb <-
function (file.rep,file.par,catch.rep,nofish=T,SBcurr_1yr=F)
{
...
# 10/7/09 SDH
# Include growth variance parameters
# Include run date and time
# 
...
# So ....
  ...
  if(is.character(file.par)){
    ...
    run_last <- file.info(file.par)$mtime
  } else {
    pr <- file.par
    run_last <- NA
  }
  ...
  SBmsytime <- msytime
  if(SBcurr_1yr) { SBmsytime <- c(msytime[2],msytime[2]) }
  ...
  if (dim(rp$TotBiomass)[1] > 1) {
    ...
    SBcurr <- mean(apply(rp$AdultBiomass[SBmsytime[1]:SBmsytime[2],],1,sum))
    } else {
    Bcurr  <- mean(rp$TotBiomass[msytime[1]:msytime[2]])
    SBcurr <- mean(rp$AdultBiomass[SBmsytime[1]:SBmsytime[2]])
...
# Compare to MSY
  ...
  resout <- list(
        ...
        Mmin = min(rp$MatAge),
        Lmin = pr$Lmin, Lmax = pr$Lmax, K = pr$K, sd_gr_gen = pr$growth_vars[1], sd_gr_length = pr$growth_vars[2],
        run_last = run_last
        )
  return(resout)
}
